...
def redact_params(params):
    if isinstance(params, dict):
        ...
        return out
    if isinstance(params, list): return [redact_params(v) for v in params]
    if isinstance(params, str) and len(params) > 256: return params[:256] + "…<truncated>"
    return params
...
class GateState:
    ...
    def authorize(self, endpoint: str, method: str, authorization: str | None) -> tuple[bool, int, str]:
        ...
        if endpoint == "upstream" and method.lower().startswith(("evm_", "anvil_", "hardhat_", "debug_")):
            return False, 403, "upstream write/admin method denied"
        return True, 200, "ok"
    ...
    def log(self, endpoint: str, method: str, code: int, allowed: bool, params, latency_ms: int) -> None:
        ...
        with self.log_path.open("a", encoding="utf-8") as fh:
            json.dump(payload, fh, sort_keys=True); fh.write("\n")

    def forward(self, endpoint: str, payload: dict) -> tuple[int, dict, int]:
        target = self.anvil_url if endpoint == "agent" else self.upstream_url
        if not target:
            return 502, {"jsonrpc":"2.0","id":payload.get("id"),"error":{"code":-32002,"message":"upstream URL unavailable"}}, 0
        ...
        try:
            ...
            status=502; out={"jsonrpc":"2.0","id":payload.get("id"),"error":{"code":-32002,"message":str(exc)}}
...
def start_anvil_if_requested() -> None:
    global _ANVIL_PROC
    if os.environ.get("START_ANVIL", "0") != "1": return
    ...
    fork_url=os.environ.get("UPSTREAM_RPC_URL"); fork_block=os.environ.get("FORK_BLOCK")
    if fork_url: args += ["--fork-url", fork_url]
    if fork_block: args += ["--fork-block-number", fork_block]
    _ANVIL_PROC=subprocess.Popen(args,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE,text=True,shell=False)


class Handler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        endpoint="agent" if self.path=="/agent" else "upstream" if self.path=="/upstream" else None
        if endpoint is None: self.send_error(404,"not found"); return
        try:
            length=int(self.headers.get("Content-Length","0")); payload=json.loads(self.rfile.read(length)); method=str(payload.get("method",""))
        except Exception:
            self._write_json(400,{"jsonrpc":"2.0","id":None,"error":{"code":-32700,"message":"invalid JSON"}}); return
        ...
        if not ok:
            ...
            self._write_json(status,{"jsonrpc":"2.0","id":payload.get("id"),"error":{"code":-32601 if status==403 else -32001,"message":reason}}); return
    ...
    def _write_json(self,status:int,payload:dict)->None:
        data=json.dumps(payload,sort_keys=True).encode(); self.send_response(status); self.send_header("Content-Type","application/json"); self.send_header("Content-Length",str(len(data))); self.end_headers(); self.wfile.write(data)

    def log_message(self,fmt:str,*args:object)->None: return


if __name__=="__main__":
    start_anvil_if_requested(); STATE=GateState(); port=int(os.environ.get("RPC_GATE_PORT","8545"))
    try: ThreadingHTTPServer(("0.0.0.0",port),Handler).serve_forever()
    finally:
        if _ANVIL_PROC is not None and _ANVIL_PROC.poll() is None: _ANVIL_PROC.terminate()
