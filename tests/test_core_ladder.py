...
def test_ladder_requires_correct_execution_artifacts_and_is_monotonic():
...
def test_l6_requires_full_certificate_binding():
    key = b"k" * 32
    c = cert(key)
    proof = LadderProof("execution", 6, ["certificate"], c, key, c.witness_sha256, "env", "records", "trace", "control")
    assert Ladder.award(construct_driver_token("driver"), make_h(), "execution", 6, proof) is True
    tampered = LadderProof("execution", 6, ["certificate"], c, key, c.witness_sha256, "env", "wrong-records", "trace", "control")
    assert Ladder.award(construct_driver_token("driver"), make_h(), "execution", 6, tampered) is False


def test_l7_requires_cross_check_certificate():
    key = b"k" * 32
    ...
    assert Ladder.award(construct_driver_token("driver"), make_h(), "production", 7, proof) is True
